...
class FetchProfileBloc extends Bloc<FetchProfileEvent, FetchProfileState> {
  FetchProfileBloc() : super(FetchProfileInitial()) {
    on<FetchProfileEvent>((event, emit) async {
      ...
      try{
        final response = await Supabase.instance.client.rpc('read_patient_profiles_by_patient_account');
        emit(FetchProfileSuccess(profiles: List<Map<String, dynamic>>.from(response)));
      }
      catch(e){
